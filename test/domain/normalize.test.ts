...
import { defaults, type Config } from "../../src/domain/config";
import {
  canonical,
  parse,
  render,
...
describe("validateMessage against commitlint rules", () => {
  const base = defaults();

  function hasWarning(cfg: Config, msg: string, fragment: string): boolean {
    return validateMessage(parse(msg)!, cfg).some((w) => w.includes(fragment));
  }
