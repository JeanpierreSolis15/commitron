...
import { parse, sanitize, validate } from "../src/message";
...
describe("validate", () => {
  ...
  it("rejects a type outside the list", () => {
    expect(() => validate(parse("banana: peel the thing")!, cfg)).toThrow();
  });

  it("accepts a configured type", () => {
    expect(validate(parse("feat: add the thing")!, cfg)).toEqual([]);
  });

  it("warns but does not fail on a long subject", () => {
    expect(validate(parse(`feat: ${"x".repeat(100)}`)!, cfg)).toHaveLength(1);
  });

  it("counts characters, not bytes", () => {
    expect(validate(parse("feat: ñññññ")!, { ...cfg, subjectMaxLength: 20 })).toEqual([]);
  });

  it("warns when the body contract is broken", () => {
    expect(validate(parse("feat: add the thing")!, { ...cfg, body: "always" })).toHaveLength(1);
    expect(
      validate(parse("feat: add the thing\n\n- one")!, { ...cfg, body: "never" }),
    ).toHaveLength(1);
  });
});
