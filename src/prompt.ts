...
export function build(cfg: Config, input: PromptInput, instructions: string): string {
  const [diff, truncated] = truncate(input.diff, cfg.maxDiffChars);
  const lines: string[] = [];
  ...
  add(
    ...
    `- Allowed types: ${cfg.types.join(", ")}. Pick exactly one, in lowercase.`,
  );
  if (cfg.scopeCase === "lower") {
    add("- The scope, when there is one, is lowercase and names an area of the codebase.");
  }
  add(
    `- Write the description in ${languageName(cfg.language)}, imperative mood: "add", never "added",`,
    '  "adds" or "adding".',
  );
  ...
  add(
    ...
    `- The whole line, type and scope included, is at most ${cfg.subjectMaxLength} characters.`,
  ...
  if (cfg.body === "never") {
    ...
    if (cfg.bodyMaxLineLength) {
      add(`- Wrap every body line at ${cfg.bodyMaxLineLength} characters.`);
    }
  }
  add(
    "",
  ...
  add("", `## Diff${truncated ? ` (truncated to ${cfg.maxDiffChars} characters)` : ""}`, diff, "");
  return lines.join("\n");
}
